$(function() {
	...
	$("#firstId input[type='submit']").on("click", function(e) {
		...
		if(operation == 'add') {  // 카테고리 추가 메뉴 선택 시
			...
			firstForm.attr("action", "modify.do");	// 서버로 변경 요청을 전송
			firstForm.submit();
		} else if(operation == 'cancel') {	// 취소 시
			if(confirm("설정을 취소하시겠습니까?")) {		// 취소를 확인
				location.href = "/aindex.do";		// 관리자 메인 페이지로 이동
			} else {								// 취소를 취소
			...
			firstForm.attr("action", "remove.do");
			firstForm.submit();
		}
	});
	// 2차 카테고리 코드 중복 확인
	...
	$("#secondId input[type='submit']").on("click", function(e) {
		...
		if(operation == 'add') {			// 추가 버튼이 눌린 경우
			...
			secondForm.attr("action", "modify.do");
			...
			secondForm.attr("action", "remove.do");
			secondForm.submit();
		}
	});
	...
	$(".firstName").on("click", function(e) {
		...
		if(display == 'block') {
			...
			// 정보를 요청(/category/getOption.do)하여 가져온다.
			var codeTag = $("#firstId input[name='code']");
			var query = { code: codeTag.val() };
			$.ajax({
				type: "POST",
				url: "/category/getOption.do",
				data: query,
				success: function(data) {
